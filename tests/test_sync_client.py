...
@patch("tws._sync.client.SyncClient._make_request")
def test_get_workflow_steps_success(mock_request, good_client):
    mock_request.return_value = [
    ...
    with good_client:
        result = good_client.get_workflow_steps("workflow-def-123")
...
@patch("tws._sync.client.SyncClient._make_request")
def test_get_workflow_steps_with_slug_filter(mock_request, good_client):
    mock_request.return_value = [
        {"id": "step-1", "slug": "step-one", "type": "action", "display_name": "Step One"},
    ]

    with good_client:
        result = good_client.get_workflow_steps("workflow-def-123", slug="step-one")
...
@patch("tws._sync.client.SyncClient._make_request")
def test_get_workflow_steps_with_type_filter(mock_request, good_client):
    mock_request.return_value = [
        {"id": "step-1", "slug": "step-one", "type": "action", "display_name": "Step One"},
    ]

    with good_client:
        result = good_client.get_workflow_steps(
...
@patch("tws._sync.client.SyncClient._make_rpc_request")
@patch("tws._sync.client.SyncClient._make_request")
def test_rerun_workflow_with_step_slug_success(mock_request, mock_rpc, good_client):
    ...
    with good_client:
        result = good_client.rerun_workflow_with_step_slug("instance-123", "step-slug-name")
...
@patch("tws._sync.client.SyncClient._make_rpc_request")
@patch("tws._sync.client.SyncClient._make_request")
def test_rerun_workflow_with_step_slug_with_overrides(mock_request, mock_rpc, good_client):
    mock_rpc.return_value = {"new_workflow_instance_id": "new-instance-456"}
    mock_request.return_value = [
    ...
    with good_client:
        good_client.rerun_workflow_with_step_slug(
            "instance-123",
            "step-slug-name",
...
@patch("tws._sync.client.SyncClient._make_rpc_request")
@patch("tws._sync.client.SyncClient._make_request")
def test_rerun_workflow_with_step_id_success(mock_request, mock_rpc, good_client):
    mock_rpc.return_value = {"new_workflow_instance_id": "new-instance-456"}
    mock_request.return_value = [
        {
            "id": "new-instance-456",
    ...
    with good_client:
        result = good_client.rerun_workflow_with_step_id("instance-123", "step-id-456")
...
@patch("tws._sync.client.SyncClient._make_rpc_request")
def test_rerun_workflow_permission_denied(mock_rpc, good_client):
    mock_request = Request("POST", "http://example.com")
    mock_response = Response(400, request=mock_request)
    ...
    with pytest.raises(ClientException) as exc_info:
        with good_client:
            good_client.rerun_workflow_with_step_slug("instance-123", "step-slug")
    assert "Permission denied to rerun this workflow instance" in str(exc_info.value)


@patch("tws._sync.client.SyncClient._make_rpc_request")
def test_rerun_workflow_not_found(mock_rpc, good_client):
    mock_request = Request("POST", "http://example.com")
    mock_response = Response(400, request=mock_request)
    ...
    with pytest.raises(ClientException) as exc_info:
        with good_client:
            good_client.rerun_workflow_with_step_id("instance-123", "step-id")
    assert "Workflow instance or step not found" in str(exc_info.value)


@patch("tws._sync.client.SyncClient._make_rpc_request")
def test_rerun_workflow_already_running(mock_rpc, good_client):
    mock_request = Request("POST", "http://example.com")
    mock_response = Response(400, request=mock_request)
...
@patch("tws._sync.client.SyncClient._make_request")
@patch("time.time")
def test_rerun_workflow_timeout(mock_time, mock_request, mock_rpc, good_client):
    mock_rpc.return_value = {"new_workflow_instance_id": "new-instance-456"}
    mock_request.return_value = [{"status": "RUNNING", "result": None}]
...
@patch("tws._sync.client.SyncClient._make_rpc_request")
@patch("tws._sync.client.SyncClient._make_request")
def test_rerun_workflow_instance_not_found_during_poll(mock_request, mock_rpc, good_client):
    mock_rpc.return_value = {"new_workflow_instance_id": "new-instance-456"}
    mock_request.return_value = []
...
@patch("tws._sync.client.SyncClient._make_rpc_request")
@patch("tws._sync.client.SyncClient._make_request")
def test_rerun_workflow_failure(mock_request, mock_rpc, good_client):
    mock_rpc.return_value = {"new_workflow_instance_id": "new-instance-456"}
    mock_request.return_value = [
        {"status": "FAILED", "result": {"error": "workflow failed"}}
    ]
